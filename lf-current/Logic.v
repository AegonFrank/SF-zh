...
Proof.
  intro X.
  assert (H: forall (l1 l2: list X), rev_append l1 l2 = rev l1 ++ l2).
  {
    induction l1.
    - reflexivity.
    - intro l2.
      simpl.
      rewrite <- (app_assoc X (rev l1) [x] l2).
      simpl.
      rewrite IHl1.
      reflexivity.
  }
...
Proof.
  ...
  unfold not.
  intros H0 X P H1 x.
  assert (H2: P x \/ (P x -> False)).
  apply H0.
  destruct H2.
  - apply H.
  - assert (H3: exists x: X, P x -> False).
    exists x.
    apply H.
    apply H1 in H3.
    destruct H3.
